...
import type { ExpertInquiry } from '@/types/database';
import { PageHeader, Card, Badge, Button, Empty } from '@/components/ui';
import styles from './expert-inquiries.module.css';

export default function ExpertInquiriesClient({ inquiries: initialInquiries }: { inquiries: ExpertInquiry[] }) {
  ...
  return (
    <>
      <PageHeader title="전문가 등록 문의" />
      <Card>
        {inquiries.length === 0 ? (
          <Empty>전문가 등록 문의가 없어요</Empty>
        ) : (
              ...
              <li key={inq.id} className={styles.item}>
                <div className={styles.header}>
                  <div className={styles.meta}>
                    ...
                    <span className={styles.contact}>{inq.phone}</span>
                    {inq.email && <span className={styles.contact}>{inq.email}</span>}
                  </div>
                  <span className={styles.date}>{timeAgo(inq.created_at)}</span>
                </div>
                {inq.affiliation && (
                  <div className={styles.affiliation}>소속/자격: {inq.affiliation}</div>
                )}
                <div className={styles.content}>{inq.content}</div>
